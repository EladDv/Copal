	...
	void EMAggregatorComponent::Reflect(AZ::ReflectContext* reflection)
	{
		...
		if (serializeContext)
		{
			serializeContext->Class<EMAggregatorComponent, AZ::Component>()
				...
				->Field("Temp Handled Tags",&EMAggregatorComponent::TempHandledTags)
			...
			if (editContext)
			{
				editContext->Class<EMAggregatorComponent>("Electromagnetic Force Aggregator Component", "This component is the base for all EM force recievers in this GEM.")
					...
					->DataElement(AZ::Edit::UIHandlers::Default, &EMAggregatorComponent::TempHandledTags, "Handled Force Tags", "List of tags handled by this aggregator")
					->Attribute(AZ::Edit::Attributes::ChangeNotify, &EMAggregatorComponent::OnTempTagsChanged)
